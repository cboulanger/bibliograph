...
class csl_name extends csl_format {
  ...
  function render($names, $mode) {
    ...
    if ( is_array( $names ) ) foreach($names as $rank => $name) {
      $count++;
    ...
    if( $auth_count == 2 and isset($this->attributes['and']) )
    {
      $text = $authors[0] . " " . $authors[1];
    } else {
      $text = implode($this->attributes['delimiter'], $authors);

      // the previous solution  doesn't work if the delimeter is the
      // same as the sort-separator
      if (isset($this->attributes['and']) && $auth_count > 1) {
        $text2 = implode($this->attributes['delimiter'],
            array_slice( $authors,0, count($authors) -1 ) ) .
...
class csl_locale  {
  ...
  function get_locales_file_name($lang) {
    ...
    return ( isset( $lang_bases[$lang] ) )
        ? file_get_contents($this->module_path.'/locale/locales-'.$lang_bases[$lang].'.xml')
  ...
  function get_locale($type, $arg1, $arg2 = NULL, $arg3 = NULL) {
    switch ($type) {
      ...
      case 'style_option':
        ...
        if(isset($option) && is_array($option) && $option[0] instanceof SimpleXMLElement) {
  ...
  public function set_style_locale($csl_doc) {
    ...
    if ($locale_nodes) {
      ...
      if (!empty($xml)) {
        ...
        $this->style_locale->registerXPathNamespace('cs', 'http://purl.org/net/xbiblio/csl');
      }
    }
  }
