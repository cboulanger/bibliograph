...
qx.Class.define("qcl.data.controller.TreeVirtual",
{
  ...
  members:
  {
     ...
     *
     * @param model {qx.core.Object|null} The model contaning the new nodes.
     * @param old {qx.core.Object|null} The old model, if any.
     */
    _applyModel: function (model, old) {
      let targetModel = this.getTarget().getDataModel();
      ...
      nodeData.forEach(function (node) {
        ...
        if (parentNodeId === undefined) {
          this.warn("Cannot add node #" + serverNodeId + ": parent node #" + serverParentId + " has not been loaded yet.");
          return;
        }
        
        if (node.isBranch) {
          clientNodeId = targetModel.addBranch(
            parentNodeId,
            node.label,
            node.bOpened,
    ...
    remapNodeIds: function () {
      let data = this.getTarget().getDataModel().getData();
      data.forEach(node => {
        try {
