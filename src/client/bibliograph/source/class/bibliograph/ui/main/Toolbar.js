...
qx.Class.define("bibliograph.ui.main.Toolbar",
{
  ...
  construct : function()
  {
    ...
    var accsMgr = app.getAccessManager();
    var permMgr = accsMgr.getPermissionManager();
    var userMgr = accsMgr.getUserManager(); 
     ...
     * Login Button
     */
    var loginButton = new qx.ui.toolbar.Button();
    ...
    accsMgr.bind("authenticatedUser", loginButton, "visibility", {
      converter : function(v) {
        return v ? 'excluded' : 'visible'
      }
    });
    loginButton.addListener("execute", function(e) {
      app.cmd("showLoginDialog");
    }, this);
    ...
    accsMgr.bind("authenticatedUser", logoutButton, "visibility", {
      converter : function(v) {
    ...
    userMgr.bind("activeUser.fullname", qxToolBarButton1, "label" );
    this.getApplication().getAccessManager().bind("authenticatedUser", qxToolBarButton1, "visibility", {
      converter : function(v) {
        return v ? 'visible' : 'excluded'
      }
    });
    ...
    //     if ( !ok ) return;
    //     this.getApplication().getRpcClient("main").send(
    //      "reset", [], function(){
    ...
    qxMenuButton10.addListener("execute", function(e) {
      this.getApplication().cmd("reportBug");
    }, this);
