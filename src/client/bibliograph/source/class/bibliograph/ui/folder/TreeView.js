  ...
  */
  members :
  {
    ...
    _editFolder : function()
    {
      ...
      this.getApplication().getRpcClient().send("folder", "edit", [this.getDatasource(), this.getSelectedNode().data.id], function() {
        this.hidePopup();
      }, this);
    },
    ...
    _emptyTrashDialog : function() {
      dialog.Dialog.confirm(this.tr("Do you really want to delete all references and folders in the trash folder? This cannot be undone."), qx.lang.Function.bind(function(answer) {
        if (answer == true)
        {
          ...
          this.getApplication().getRpcClient("folder").send( "purge", [this.getDatasource()], function()
          {
            this.showPopup(this.tr("Purging deleted references ..."))
            this.getApplication().getRpcClient("reference").send( "purge", [this.getDatasource()], function() {
              this.hidePopup();
            }, this);
          }, this);
        }
