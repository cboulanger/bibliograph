...
class TreeController extends AppController implements ITreeController
{
  ...
  /**
   * Returns the tree node model class
   * @param string $datasource
   * @return string
   */
  protected function getNodeModelClass( $datasource )
  {
    return Datasource::getInstanceFor($datasource)::getClassFor($this->modelType);
  }
  ...
  */
   ...
   *   the new record.
   * @param object|array|null $options object with the properties label, position and parentId
   * @return mixed Id of the newly created record.
   * @override
   */
  function createNode( $datasource, $options=null )
  {
    $class = $this->getNodeModelClass( $datasource );
    $model = new $class((array)$options);
    $model->save();
    $parent = $class::findOne(['parentId' => ((object)$model)->parentId ]);
    $model->reorder();
  ...
  {
    $model = $this->getNodeModelClass( $datasource );
  ...
  {
    $model = $this->getNodeModelClass( $datasource, $parentId );
  ...
  function changePosition ( $datasource, $nodeId, $position )
  {
     ...
     */
    $model = $this->getNodeModelClass( $datasource, $nodeId );
