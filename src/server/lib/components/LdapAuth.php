...
class LdapAuth extends \yii\base\Component
{
   ...
   *
   * @return array
   *    An associated array with the keys 'enabled' (bool), 'connection' (bool)
   *   and
   *    'error' (string).
   */
  ...
  /**
   * Identifies a user at a remote LDAP server and creates local user if they
   * exist. Returns the user model instance
   * @param $username
      ...
      if ($defaultRole) {
        ...
        if ($role and !$user->getUserRoles()->where($condition)->exists()) {
          ...
          try {
            ...
            // ignore integrity constraint violation
            // !$user->getUserRoles()->where($condition)->exists() seems not to catch existing record(s)
          }
        }
      }
  ...
  /**
   * Returns an array LDAP records that match a certain name, or an empty array
   * if none matches
   * @param $identifier
   * @return array Array of associative arrays with the keys "namedId" (the
   *   LDAP user id) and "name" (taken from the CN attribute)
   * @throws \RuntimeException
